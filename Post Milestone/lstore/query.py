...
class Query:
    ...
    def select_version(self, index_value, index_column, query_columns, relative_version):
        ...
        for i in range(len(query_columns)):
            if query_columns[i] == 1:
                arr.append(i)
        baseRecord_RID = self.table.index.locate(index_column, index_value)
        selected_record = self.table.timetravel_read(baseRecord_RID, arr, relative_version)
    ...
    def sum_version(self, start_range, end_range, aggregate_column_index,relative_version):
        ...
        for key in range(start_range, end_range + 1):
            rid = self.table.index.locate(aggregate_column_index, key)
            if rid is not None:
                total += self.table.timetravel_read(rid, [aggregate_column_index], relative_version)[0].columns[0]
        return total
