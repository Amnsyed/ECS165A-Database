...
for _ in range(number_of_updates):
    for key in keys:
        ...
        updates.append(updated_records[key])
keys = sorted(list(records.keys()))
...
for key in keys:
    ...
    for i, column in enumerate(record.columns):
        if column != updates[2][i]:
            error = True
    if error:
        print('select error on', key, ':', record, ', correct:', updates[2], 'version: 0')
...
for key in keys:
    ...
    error = False
    for i, column in enumerate(record.columns):
        if column != updates[1][i]:
            error = True
    if error:
        print('select error on', key, ':', record, ', correct:', updates[1], 'version: -1')
...
for key in keys:
    ...
    error = False
    for i, column in enumerate(record.columns):
        if column != updates[0][i]:
            error = True
    if error:
        print('select error on', key, ':', record, ', correct:', updates[0], 'version: -2')
...
for key in keys:
    ...
    for i, column in enumerate(record.columns):
        if column != records[key][i]:
            error = True
    if error:
        print('select error on', key, ':', record, ', correct:', records[key], 'version: -3')
...
for key in keys:
    ...
    for i, column in enumerate(record.columns):
        if column != updated_records[key][i]:
            error = True
    if error:
        print('select error on', key, ':', record, ', correct:', records[key], 'version: 0')
print("Select for version 0 finished")
