...
class PageRange:
    ...
    def addNewRecordValues(self):
        ...
        if self.last_base_offset == 4096:
            ...
            self.last_base_page += 1
        page, offset = self.last_base_page, self.last_base_offset
        self.last_base_offset += 8
        return (page, offset)

    def addNewRecord(self, rid, last_base_page, last_base_offset, *columns):
        # return a tuple (base page #, offset #)
        
        page = self.base_page_array[last_base_page]
        # add the record values to pages
        for i in range(self.num_columns):
            self.table.bufferpool.get(page[i], self.table.table_path, (self.page_range_num, last_base_page, i, self.total_bp_columns), 'bp')
            page[i].write(columns[i], last_base_offset)
            self.table.bufferpool.decrement_and_markDirty(page[i])
        # add rid, indirection to page and schema encoding
        self.table.bufferpool.get(page[self.num_columns], self.table.table_path, (self.page_range_num, last_base_page, self.num_columns, self.total_bp_columns), 'bp')
        page[self.num_columns].write(rid, last_base_offset)
        self.table.bufferpool.get(page[self.num_columns + 1], self.table.table_path, (self.page_range_num, last_base_page, self.num_columns + 1, self.total_bp_columns), 'bp')
        page[self.num_columns + 1].write(0, last_base_offset)
        self.table.bufferpool.get(page[self.num_columns + 2], self.table.table_path, (self.page_range_num, last_base_page, self.num_columns + 2, self.total_bp_columns), 'bp')
        page[self.num_columns + 2].write(0, last_base_offset)
    ...
    def updateRecordValues(self):
        if self.last_tail_offset == 4096:
            ...
            self.last_tail_page += 1
        page, offset = self.last_tail_page, self.last_tail_offset
        self.last_tail_offset += 8
        return page, offset

    def updateRecord(self, base_record_rid, base_page_num, offset, last_tail_page, last_tail_offset, *columns):
        new_rid = self.table.generate_unique_rid()
        # add new tail page if last one out of capacity
        
        
        page = self.tail_page_array[last_tail_page]
        self.tail_page_directory[new_rid] = (last_tail_page, last_tail_offset)
        ...
        if self.base_page_array[base_page_num][self.num_columns + 1].read(offset) == 0:
            ...
            for i in range(self.num_columns):
                self.table.bufferpool.get(page[i], self.table.table_path, (self.page_range_num, last_tail_page, i, self.total_tp_columns), 'tp')
                if columns[i] == None:
                    page[i].write(0 , last_tail_offset)
                else:
                    page[i].write(columns[i], last_tail_offset)
                    schema_encoding = schema_encoding[:i + 1] + '1' + schema_encoding[i+2:]
                self.table.bufferpool.decrement_and_markDirty(page[i])
            self.table.bufferpool.get(page[self.num_columns], self.table.table_path, (self.page_range_num, last_tail_page, self.num_columns, self.total_tp_columns), 'tp')
            self.table.bufferpool.get(page[self.num_columns + 1], self.table.table_path, (self.page_range_num, last_tail_page, self.num_columns + 1, self.total_tp_columns), 'tp')
            self.table.bufferpool.get(page[self.num_columns + 2], self.table.table_path, (self.page_range_num, last_tail_page, self.num_columns + 2, self.total_tp_columns), 'tp')
            page[self.num_columns].write(new_rid, last_tail_offset)
            page[self.num_columns + 1].write(self.base_page_array[base_page_num][self.num_columns].read(offset), last_tail_offset)
            page[self.num_columns + 2].write(base_record_rid, last_tail_offset)
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns])
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns + 1])
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns + 2])
        else:
            ...
            for i in range(self.num_columns):
                ...
                if columns[i] is not None:
                    ...
                    page[i].write(prev_tail_page[i].read(prev_tail_record_location[1]), last_tail_offset)
            ...
            self.table.bufferpool.get(page[self.num_columns + 2], self.table.table_path, (self.page_range_num, last_tail_page, self.num_columns + 2, self.total_tp_columns), 'tp')
            page[self.num_columns].write(new_rid, last_tail_offset)
            page[self.num_columns + 1].write(prev_tail_rid, last_tail_offset)
            page[self.num_columns + 2].write(base_record_rid, last_tail_offset)
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns])
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns + 1])
            self.table.bufferpool.decrement_and_markDirty(page[self.num_columns + 2])
