...
class Table:
    ...
    def insert(self, *columns):
        ...
        page_directory_val = None
        if pagerange_len > 0 and self.pagerange_array[pagerange_len - 1].hasCapacity():
            page_directory_val = self.pagerange_array[pagerange_len - 1].addNewRecordValues()
        else:
        ...
        self.pagerange_array[pagerange_len - 1].addNewRecord(rid_in, page_directory_val[0], page_directory_val[1], *columns)
        self.index.insert(self.key, columns[self.key], rid_in)
    ...
    def update(self, primary_key, *columns):
        ...
        tail_page, tail_offset = self.pagerange_array[params[0]].updateRecordValues()
        self.update_record.release()
        self.pagerange_array[params[0]].updateRecord(temp_rid, basepage, offset, tail_page, tail_offset, *columns)
    ...
    def read(self, rid, query_columns):
        ...
        record_columns = self.pagerange_array[pagerange_num].readRecord(basepage_num, offset, query_columns)
        #print(rid, record_columns)
    ...
    def generate_unique_rid(self):
        self.rid_generate.acquire()
        ret = self.rid_temp
        self.rid_temp += 1
        self.rid_generate.release()
        return ret
